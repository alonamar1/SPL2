 ...
 * {@link #sendBroadcast(bgu.spl.mics.Broadcast)},
 * etc.) they can use. When subscribing to message-types, the derived class also
...
public abstract class MicroService implements Runnable {
     ...
     * {@code callback}. This means two things: 1. Subscribe to events in the
     * singleton event-bus using the supplied {@code type} 2. Store the
     * {@code callback} so that when events of type {@code type} are received it
     * will be called.
     * <p>
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        messagecallbacks.put(type, callback); // Store the callback
        MessageBusImpl.getInstance().subscribeEvent(type, this); // Register with MessageBus
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        ...
        messagecallbacks.put(type, callback); // Store the callback
        MessageBusImpl.getInstance().subscribeBroadcast(type, this); // Register with MessageBus
    }

    /**
     * Sends the event {@code e} using the message-bus and receive a
     * {@link Future<T>} object that may be resolved to hold a result. This
     * method must be Non-Blocking since there may be events which do not
     * require any response and resolving.
     * <p>
     * 
     * @param <T> The type of the expected result of the request {@code e}
     * @param e   The event to send
     * @return {@link Future<T>} object that may be resolved later by a
     *         different micro-service processing this event. null in case no
     *         micro-service has subscribed to {@code e.getClass()}.
     */
    protected final <T> Future<T> sendEvent(Event<T> e) {

        return MessageBusImpl.getInstance().sendEvent(e); // Send event through MessageBus
    }

    /**
     * A Micro-Service calls this m\ethod in order to send the broadcast message
    ...
    protected final void sendBroadcast(Broadcast b) {
        MessageBusImpl.getInstance().sendBroadcast(b); // Send broadcast through MessageBus
    ...
    public final void run() {
        ...
        while (!terminated && !Thread.currentThread().isInterrupted()) { // Run until termination
            try {
                ...
                Callback<Message> callback = (Callback<Message>) messagecallbacks.get(msg.getClass()); // Find the
                                                                                                       // corresponding
                                                                                                       // callback
                if (callback != null) {
